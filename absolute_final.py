"""
Absolute Final Hail Mary
========================
- Bypasses PyArrow Dataset partition explicit bug by loading columns separately.
- Executes 5-Fold Micro-Learning.
- Sequence smooths over test dataset natively.
"""
...
# Load TARGET from train
meta_t = pd.read_parquet("train.parquet", columns=["TARGET"])
meta_g = pd.read_parquet("train.parquet", columns=["CV_GROUP"])
y = meta_t["TARGET"].values.astype(np.float32)
groups = meta_g["CV_GROUP"].values
n_train = len(y)
del meta_t, meta_g; gc.collect()

# Bypass PyArrow bug by loading ID and CV_GROUP separately!
test_id = pd.read_parquet("test.parquet", columns=["ID"])
test_cv = pd.read_parquet("test.parquet", columns=["CV_GROUP"])
test_meta = pd.concat([test_id, test_cv], axis=1)
test_ids = test_meta["ID"].values
n_test = len(test_ids)
del test_id, test_cv; gc.collect()
...
test_meta = test_meta.sort_values("CV_GROUP")
