...
Anti-Drift Pipeline 
===================
1. Isolates feature distribution drift via robust Adversarial Validation.
2. Purges the top 80 most toxic shifted features (like `Price`).
3. Trains a deep robust LightGBM regressor on the remaining stationary features.
"""
...
log("Phase 1: Setup & Corrupted Column Bypass...")
...
log(f"Phase 2: Adversarial Drift Profiling (Features: {len(safe_feats)})...")
# Build a fast random subset to profile drift securely
idx_tr = np.random.choice(n_train, 250000, replace=False)
idx_te = np.random.choice(n_test, 250000, replace=False)

adv_importances = {}
batch_size = 50
log("  Iteratively mapping distribution shift...")
for i in range(0, len(safe_feats), batch_size):
    b = safe_feats[i:i+batch_size]
    # Load chunks to save memory
    ...
    preds = adv_model.predict_proba(xva)[:, 1]
    auc = roc_auc_score(yva, preds)
    log(f"    Batch {i}-{i+batch_size} AUC: {auc:.4f}")
    
    # Map importances directly cleanly
...
# We strictly cull any feature that has high adversarial importance (drift)
...
log(f"\nPhase 3: Generating Clean Stationary Subset...")
log(f"  Dropping {TOXIC_COUNT} toxic variables. Top 5 purged: {[x[0] for x in adversarial_ranking[:5]]}")

clean_feats = [f for f in safe_feats if f not in toxic_features]

# Pick Top 120 absolute correlated features from the CLEAN set
log("  Computing Stationary Correlations...")
y_c = y - y.mean()
corrs = {}
...
log(f"  Stationary set loaded: {len(selected)} features.")
del corrs, y_c; gc.collect()

log("\nPhase 4: Constructing Safe Matrices...")
...
log("\nPhase 5: Deep Learning (Anti-Drift)...")
N_FOLDS = 6
...
log(f"  Clean Final OOF R2: {best_scale_r2:.6f} with Scale: {best_scale:.2f}")

log("\nPhase 7: Mapping Final Sequence Targets...")
...
test_meta["RAW_PRED"] = test_blend * best_scale

# Global safe sequential smoothing
...
# Blend 90% Raw Predict, 10% Smoothing
test_meta["TARGET"] = 0.90 * test_meta["RAW_PRED"] + 0.05 * test_meta["pred_prev"] + 0.05 * test_meta["pred_next"]

test_sub = test_meta[["ID", "TARGET"]]
test_sub.to_csv("submission_anti_drift.csv", index=False)

log(f"\nSaved submission_anti_drift.csv ({len(test_sub)} rows)")
log("DONE! Adversarial Drift Cleaned and Ready.")
