"""
Ultimate Seed Blend Final Pipeline
==================================
Based purely on the proven `0.00041` architecture.
- 6-Fold GroupKFold
- Toxic Adversarial Drop = 85
- Huber + Fair losses
- Seed Engine = [42, 2024, 777]
- True sequence smoothing (0.90 / 0.05 / 0.05)
"""
...
log("Phase 2: Adversarial Surrogate (Toxic Isolation)...")
np.random.seed(42)
...
for i in range(0, len(safe_feats), 50):
    ...
    for col, imp in zip(b, adv_model.feature_importances_):
...
log("Phase 3: Stationary Subsetting...")
y_c = y - y.mean()
corrs = {}
...
log("Phase 4: Matrix Bootstrapping...")
...
log("\nPhase 5: Iterative Seed Ensemble Deep Learning...")

N_FOLDS = 6
gkf = GroupKFold(n_splits=N_FOLDS)

all_seed_oof_blend = np.zeros(n_train, np.float32)
global_test_blend = np.zeros(n_test, np.float32)
X_test = build_matrix("test.parquet", test_col_set, n_test, selected)

best_scale = 1.10 # Hardcoded optimal scale from 0.00041 run

for current_seed in SEEDS:
    log(f" => Initiating SEED Run: {current_seed}")
    ...
    CONFIGS = [
        {
            ...
            "params": {
                ...
                "reg_alpha": 0.1, "reg_lambda": 1.0, "verbose": -1, "n_jobs": -1, "random_state": current_seed
            }
        },
        {
            "name": "Fair",
            "params": {
                ...
                "reg_alpha": 2.0, "reg_lambda": 5.0, "verbose": -1, "n_jobs": -1, "random_state": current_seed
            }
        }
    ]
    
    seed_oof_preds = {cfg["name"]: np.zeros(n_train, np.float32) for cfg in CONFIGS}
    
    for cfg in CONFIGS:
        log(f"    Training {cfg['name']}...")
        w = 0.5 # Equal weighting for Huber and Fair
        for fi, (tr, va) in enumerate(gkf.split(X_train, y_clip, groups)):
            ...
            seed_oof_preds[cfg["name"]][va] = bst.predict(X_train[va])
            
            # Accumulate exactly to global test prediction correctly
            chunk_size = 50000
            for start_idx in range(0, n_test, chunk_size):
                end_idx = min(start_idx + chunk_size, n_test)
                # Weighted blend, divided by Folds and further divided by total number of seeds
                global_test_blend[start_idx:end_idx] += (bst.predict(X_test[start_idx:end_idx]) * w) / (N_FOLDS * len(SEEDS))
            
            del dt, dv, bst; gc.collect()
            
    # Add this seed's blended OOF to our global OOF tracker
    seed_blend = seed_oof_preds["Huber"] * 0.5 + seed_oof_preds["Fair"] * 0.5
    all_seed_oof_blend += seed_blend / len(SEEDS)
    log(f"    Seed {current_seed} Local R2: {r2_score(y, seed_blend):.6f}")

log(f"\nFinal Global Multi-Seed Ensemble OOF R2: {r2_score(y, all_seed_oof_blend):.6f}")

log("\nPhase 6: Hardcoded True Sequence Smoothing...")
# CRITICAL: We preserve the 1.10 scale precisely and strictly smooth 0.90, 0.05, 0.05
test_meta["RAW_PRED"] = global_test_blend * best_scale
...
test_meta["TARGET"] = 0.90 * test_meta["RAW_PRED"] + 0.05 * test_meta["pred_prev"] + 0.05 * test_meta["pred_next"]

test_sub = test_meta[["ID", "TARGET"]]
test_sub.to_csv("submission_final_ticket.csv", index=False)

log(f"\nCompleted successfully! Multi-Seed prediction stored.")
log(f"Saved submission_final_ticket.csv ({len(test_sub)} rows)")
