...
- 10-Fold 
- Lower Learning Rate 
- Bypasses test set corrupted columns (CV_GROUP/SO3_T).
- Global sequential smoothing.
"""
...
# DO NOT LOAD CV_GROUP OR SO3_T FROM TEST. THEY CORRUPT PYARROW.
...
def build_matrix(path, avail, n_rows, raw_feats):
    eng_needs = set(["Price", "Price_LagT1", "Price_LagT2"])
...
log("\nStep 4: Training 6-Fold Deep Ensemble...")
...
for i, cfg in enumerate(CONFIGS):
    ...
    for ms in models[name]:
        ...
        # Avoid memory crash by predicting in chunks
        chunk_size = 50000
...
# Without CV_GROUP mapping available, we apply a gentle global smooth 
# assuming Kaggle natively sorted the test IDs sequentially
...
# 90% strict ML, 10% momentum smoothing
test_meta["TARGET"] = 0.90 * test_meta["RAW_PRED"] + 0.05 * test_meta["pred_prev"] + 0.05 * test_meta["pred_next"]

test_sub = test_meta[["ID", "TARGET"]]
test_sub.to_csv("submission_perfect.csv", index=False)
log(f"Saved submission_perfect.csv ({len(test_sub)} rows)")
log("DONE!")
